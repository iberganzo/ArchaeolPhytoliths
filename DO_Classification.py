...
# by Iban Berganzo-Besga1, Hector A. Orengo1, Felipe Lumbreras2, Paloma Aliende1, Monica N. Ramsey3
...
# Divide code
    ...
    if not os.path.exists('Divided'):
        ...
        for filenameR in glob.glob("Divided/*"):
            os.remove(filenameR)
    pathPredDivided=os.path.join(path,'Divided')
    ...
    for iFile in range(0,len(file_listPred),1):
      image = Image.open('Divided/%s' %file_listPred[iFile])
