...
pathDataImg=os.path.join(path,'Pred/DataToClassify')
for filenameR in glob.glob("Pred/DataToClassify/.*"):
        os.remove(filenameR)
...
# Divide code
    ...
    if not os.path.exists('Pred/Results'):
        os.mkdir('Pred/Results')

    imgOriginal = cv2.imread('Pred/DataToClassify/%s' %file_list[i1])
    height, width, channels = imgOriginal.shape
    pathResult=os.path.join(path,'Pred/Results/%s' %file_list[i1])
    ...
    if not os.path.exists('Pred/PredDivided'):
        os.mkdir('Pred/PredDivided')
    else:
        for filenameR in glob.glob("Pred/PredDivided/*"):
            os.remove(filenameR)
    pathPredDivided=os.path.join(path,'Pred/PredDivided')
    ...
    model = keras.models.load_model('Composite_model_5C_VGG19_DA_e50_b12.h5')

    # Load prediction data

    pathDataPred=os.path.join(path,'Pred/PredDivided/')
    for filenameR in glob.glob("Pred/PredDivided/.*"):
        os.remove(filenameR)
    ...
    for iFile in range(0,len(file_listPred),1):
      image = Image.open('Pred/PredDivided/%s' %file_listPred[iFile])
    ...
    for iPred in range(0, len(pred), 1):
        ...
        imgResult = cv2.imread('Pred/Results/%s' %file_list[i1])
